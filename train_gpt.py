...
def quantize_int6_gptq(weight: Tensor, hessian: Tensor, clip_range: int = 31,
    ...
    """GPTQ int6 quantization with Cholesky error compensation."""
    ...
    # Column ordering by Hessian diagonal (largest first)
    perm = torch.argsort(torch.diag(H), descending=True)
    inv_perm = torch.argsort(perm)
    W = W[:, perm]
    H = H[perm][:, perm]
    # Full inverse Hessian (we need it for inter-block error propagation)
    try:
        Hinv = torch.linalg.inv(H)
    except torch.linalg.LinAlgError:
        H.diagonal().add_(damp * 10)
        Hinv = torch.linalg.inv(H)
    # Find best clipping percentile
    best_q, best_s, best_err = None, None, float('inf')
    for pct in [0.999, 0.9995, 0.9999, 0.99999, 1.0]:
        ...
        s = (row_clip / clip_range).clamp_min(1.0 / clip_range).to(torch.float16)
        sf = s.float()[:, None]
        Q = torch.zeros_like(Wc)
        for col0 in range(0, ncols, block_size):
            col1 = min(col0 + block_size, ncols)
            Hinv_block = Hinv[col0:col1, col0:col1]
            Err = torch.zeros(nrows, col1 - col0, device=W.device)
            for j in range(col1 - col0):
                w_col = Wc[:, col0 + j]
                q_col = torch.clamp(torch.round(w_col / sf.squeeze(1)), -clip_range, clip_range)
                Q[:, col0 + j] = q_col
                err = (w_col - q_col * sf.squeeze(1)) / Hinv_block[j, j]
                Err[:, j] = err
                # Propagate error to remaining columns in block
                if j + 1 < col1 - col0:
                    Wc[:, col0 + j + 1:col1] -= err.unsqueeze(1) * Hinv_block[j, j + 1:col1].unsqueeze(0)
            # Propagate block error to remaining columns
            if col1 < ncols:
                Wc[:, col1:] -= Err @ Hinv[col0:col1, col1:]
        recon = Q.float() * sf
        recon_orig = recon[:, inv_perm]
        err = (weight.float() - recon_orig).pow(2).mean().item()
        if err < best_err:
            best_q = Q[:, inv_perm].to(torch.int8)
            best_s = s
            best_err = err
    return best_q, best_s

def collect_hessians(model, train_loader, args, device: torch.device,
                     t0_seconds: float, max_seconds: float,
                     log0=print) -> dict[str, Tensor]:
    """Collect H = X^T X for each linear layer using training data forward passes.

    For banked models, we hook Block.forward and MLP.forward to capture activations
    and map them to unbanked weight names (blocks.{i}.attn.c_q.weight, etc).
    """
    hessians: dict[str, Tensor] = {}
    hooks = []
    n_phys = model.num_layers
    ...
    def block_hook(block_idx: int):
        """Hook on Block.forward to capture inputs to Q/K/V linear layers."""
        def hook_fn(module, args_tuple, output):
            x = args_tuple[0]
            pi = model.v2p[block_idx]
            # Attn input: attn_norm(mix(x, x0)) * ln_scale_factor
            mix = module.resid_mix.to(dtype=x.dtype)
            x0 = args_tuple[1]
            x_in = mix[0][None, None, :] * x + mix[1][None, None, :] * x0
            attn_input = F.rms_norm(x_in, (x_in.size(-1),)) * module.ln_scale_factor
            # Q, K, V share the same input
            _accum(f"blocks.{pi}.attn.c_q.weight", attn_input)
            _accum(f"blocks.{pi}.attn.c_k.weight", attn_input)
            _accum(f"blocks.{pi}.attn.c_v.weight", attn_input)
        return hook_fn

    def mlp_hook(block_idx: int):
        """Hook on MLP.forward to capture mlp_up input."""
        def hook_fn(module, args_tuple, output):
            x = args_tuple[0]
            pi = model.v2p[block_idx]
            _accum(f"blocks.{pi}.mlp.fc.weight", x)
        return hook_fn

    for i in range(model.virtual_num_layers):
        hooks.append(model.blocks[i].register_forward_hook(block_hook(i)))
        hooks.append(model.blocks[i].mlp.register_forward_hook(mlp_hook(i)))

    model.eval()
    n_batches = 0
    with torch.inference_mode():
        for _ in range(args.gptq_calib_batches):
            if time.perf_counter() - t0_seconds >= max_seconds:
                log0(f"gptq:hessian_collection interrupted by wallclock at batch {n_batches}/{args.gptq_calib_batches}")
                break
...
def mixed_quantize_int6(state_dict: dict[str, Tensor], int6_cats: set[str],
    ...
    for name, tensor in state_dict.items():
        ...
        if cat in int6_cats and t.ndim >= 1:
            if hessians is not None and name in hessians and t.ndim == 2:
                q, s = quantize_int6_gptq(t, hessians[name], block_size=gptq_block_size)
                gptq_count += 1
            else:
    ...
    if hessians is not None:
        print(f"gptq:quantized {gptq_count} layers with GPTQ, {len(state_dict) - gptq_count} with fallback")
    return result, meta
...
def main() -> None:
    ...
    if not args.eval_only:
        ...
        t0 = time.perf_counter()
        t0_seconds_global = t0  # global start for GPTQ wallclock check
        step = 0
        while True:
            ...
            if distributed:
                grads_to_reduce = [p.grad for p in replicated_params if p.grad is not None]
                if grads_to_reduce:
                    coalesced = torch._utils._flatten_dense_tensors(grads_to_reduce)
                    dist.all_reduce(coalesced, op=dist.ReduceOp.AVG)
                    for buf, synced in zip(grads_to_reduce, torch._utils._unflatten_dense_tensors(coalesced, grads_to_reduce)):
                        buf.copy_(synced)
            optimizer_tok.step()
        ...
        if args.gptq_enabled:
            ...
            gptq_hessians = collect_hessians(
                base_model, train_loader, args, device,
                t0_seconds=t0_seconds_global,
                max_seconds=args.max_wallclock_seconds,
                log0=log0,
            )
