...
class Hyperparameters:
    ...
    ngram_order = int(os.environ.get("NGRAM_ORDER", 3))  # 2=bigram, 3=trigram
    ngram_inject_layers = os.environ.get("NGRAM_INJECT_LAYERS", "")  # e.g. "6" to re-inject at virtual layer 6
...
class NgramHashEmbedding(nn.Module):
    def __init__(self, bigram_vocab_size: int, bigram_dim: int, model_dim: int, ngram_order: int = 2):
        super().__init__()
    ...
    def ngram_hash(self, tokens: Tensor) -> Tensor:
        ...
        if self.ngram_order >= 3 and t.shape[-1] > 2:
...
class GPT(nn.Module):
    def __init__(
        ...
        bigram_dim: int = 128,
        ngram_order: int = 2,
        ngram_inject_layers: list[int] | None = None,
        xsa_last_n: int = 0,
        rope_dims: int = 0,
        ln_scale: bool = False,
        dtg: bool = False,
        ...
        self.bigram = NgramHashEmbedding(bigram_vocab_size, bigram_dim, model_dim, ngram_order=ngram_order) if bigram_vocab_size > 0 else None
        self.ngram_inject_layers = set(ngram_inject_layers) if ngram_inject_layers else set()
        if self.ngram_inject_layers and self.bigram is not None:
            self.ngram_inject_scales = nn.ParameterDict({
                str(l): nn.Parameter(torch.tensor(0.01, dtype=torch.float32))
                for l in sorted(self.ngram_inject_layers)
            })
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            if i in self.ngram_inject_layers and ngram_emb is not None:
                x = x + ngram_emb * self.ngram_inject_scales[str(i)].to(dtype=x.dtype)
            pi = v2p[i]
        ...
        for i in range(self.num_decoder_layers):
            ...
            pi = v2p[bi]
            if bi in self.ngram_inject_layers and ngram_emb is not None:
                x = x + ngram_emb * self.ngram_inject_scales[str(bi)].to(dtype=x.dtype)
            if skips:
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        ngram_inject_layers=([int(x) for x in args.ngram_inject_layers.split(",") if x.strip()] or None),
    ...
    eval_model = GPT(
        ...
        bigram_vocab_size=args.bigram_vocab_size, bigram_dim=args.bigram_dim, ngram_order=args.ngram_order,
        ngram_inject_layers=([int(x) for x in args.ngram_inject_layers.split(",") if x.strip()] or None),
        xsa_last_n=args.xsa_last_n,
