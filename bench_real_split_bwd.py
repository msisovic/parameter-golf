"""Bench the ACTUAL split-path backward: bwd CE kernel + cuBLAS matmuls."""
import torch
import triton
...
print("=== ACTUAL split-path backward ===")
...
ms_ce = bench(run_bwd_ce, "bwd CE kernel (grad_logits from stored logits)")

def run_dx_cublas():
    return grad_logits @ w
ms_dx = bench(run_dx_cublas, "cuBLAS dx (grad_logits @ w)")

def run_dw_cublas():
    return grad_logits.t() @ x
ms_dw = bench(run_dw_cublas, "cuBLAS dw (grad_logits.T @ x)")

def run_full_split_bwd():
    fused_softcap_ce_bwd_kernel[(M,)](
        grad_logits, grad_out, lse, logits, targets,
        logits.stride(0), logits.stride(1),
        grad_logits.stride(0), grad_logits.stride(1),
    ...
    gi = grad_logits @ w
    gw = grad_logits.t() @ x
ms_total = bench(run_full_split_bwd, "TOTAL split bwd (CE + 2x cuBLAS)")

print(f"\n=== Nologit recompute bwd for comparison ===")
...
ms_nologit = bench(run_nologit_bwd, "TOTAL nologit bwd (dx_recompute + dw_recompute)")

print(f"\n  split bwd: {ms_total:.3f} ms")
print(f"  nologit bwd: {ms_nologit:.3f} ms")
print(f"  delta: {ms_nologit - ms_total:+.3f} ms")
