...
class Hyperparameters:
    ...
    bump_obs_enabled = bool(int(os.environ.get("BUMP_OBS_ENABLED", "0")))
    bump_obs_start_step = int(os.environ.get("BUMP_OBS_START_STEP", -1))
    bump_obs_end_step = int(os.environ.get("BUMP_OBS_END_STEP", -1))
    bump_obs_n = int(os.environ.get("BUMP_OBS_N", 20))
    bump_obs_post_late_offset = int(os.environ.get("BUMP_OBS_POST_LATE_OFFSET", 100))
    bump_obs_every = int(os.environ.get("BUMP_OBS_EVERY", 1))
    bump_obs_probe_batches = int(os.environ.get("BUMP_OBS_PROBE_BATCHES", 4))
    bump_obs_counterfactual = bool(int(os.environ.get("BUMP_OBS_COUNTERFACTUAL", "1")))
...
class Rotary(nn.Module):
    ...
    def _build_cache(self, seq_len, device, yarn_seq_len=None):
        ...
        rd = self.rope_dims
        if self.yarn and yarn_seq_len > self.train_seq_len:
            scale = yarn_seq_len / self.train_seq_len
    ...
    def forward(self, seq_len, device, dtype, yarn_seq_len=None, cache_slot=None):
        if cache_slot == 0:
            if self._fixed_short_cos is None or self._fixed_short_seq_len < seq_len:
                raise RuntimeError(
                    f"rotary short slot not primed for seq_len={seq_len}, cached={self._fixed_short_seq_len}"
                )
            return (
                self._fixed_short_cos[:, :seq_len].to(dtype=dtype),
                self._fixed_short_sin[:, :seq_len].to(dtype=dtype),
            )
        if cache_slot == 1:
            if self._fixed_long_cos is None or self._fixed_long_seq_len < seq_len:
                raise RuntimeError(
                    f"rotary long slot not primed for seq_len={seq_len}, cached={self._fixed_long_seq_len}"
                )
            return (
...
class CausalSelfAttention(nn.Module):
    ...
    def forward(self, x, q_w, k_w, v_w, out_w, cu_seqlens=None, max_seqlen=0, rotary_slot=None):
        ...
        cos, sin = self.rotary(
            seqlen, x.device, q.dtype, yarn_seq_len=max_seqlen or seqlen, cache_slot=rotary_slot
        )
...
class GPT(nn.Module):
    ...
    def clamp_identity_aux_skips_(self):
        if not self.loop_identity_topology:
            return
        start = self.loop_identity_aux_skip_start
        end = start + self.loop_identity_aux_skip_count
        with torch.no_grad():
    ...
    def _parallel_block(
        ...
        cu_seqlens=None, max_seqlen=0, rotary_slot=None,
    ):
    ...
    def forward_logits(self, input_ids, cu_seqlens=None, max_seqlen=0, rotary_slot=None):
        ...
        x0 = x
        if self.looping_active and self.loop_identity_topology:
            identity_skips = [None] * self.num_skip_weights
            enc_iter = (
                (0, -1, -1, 4),
                (1, -1, -1, 3),
                (2, -1, -1, 2),
            ...
            for i, extra_pass, loop_local_idx, skip_slot in enc_iter:
            ...
            for i, extra_pass, loop_local_idx, skip_slot in dec_iter:
                q_w, k_w, v_w, out_w, up_w, down_w = self._bank_weights(i)
                if i >= psl and psl > 0:
                    if lane0 is None:
                        lane0 = x
                        lane1 = x
                    if skip_slot >= 0:
                        skip = identity_skips[skip_slot]
                        w = self.skip_weights[skip_slot].to(dtype=lane0.dtype)[None, None, :]
                        if self.skip_gates is not None:
                            g = torch.sigmoid(self.skip_gates[skip_slot].to(dtype=lane0.dtype))[None, None, :]
                    ...
                    if skip_slot >= 0:
                        skip = identity_skips[skip_slot]
                        scaled_skip = (
                            self.skip_weights[skip_slot].to(dtype=x.dtype)[None, None, :]
                            * skip
                        )
                        if self.skip_gates is not None:
                            g = torch.sigmoid(self.skip_gates[skip_slot].to(dtype=x.dtype))[None, None, :]
    ...
    def forward(self, input_ids, target_ids, cu_seqlens=None, max_seqlen=0, rotary_slot=None):
        logits = self.forward_logits(
            input_ids, cu_seqlens=cu_seqlens, max_seqlen=max_seqlen, rotary_slot=rotary_slot
        )
...
def _prime_rotary_fixed_slots(model, slot_specs, device, dtype=torch.bfloat16):
    for block in model.blocks:
        rotary = block.attn.rotary
        for slot, seq_len, yarn_seq_len in slot_specs:
            rotary.prime_fixed_slot(
                slot, seq_len, device, dtype, yarn_seq_len=yarn_seq_len
            )
...
def train_model(h, device, val_data):
    ...
    _prime_rotary_fixed_slots(
        base_model,
        (
            (0, rotary_slot_tokens, short_train_seq_len),
            (1, rotary_slot_tokens, long_train_seq_len),
        ),
        device,
    )
    ...
    def should_enable_looping(step, frac):
        if h.loop_ramp_steps > 0:
            return loop_policy_probability(step, frac) >= 1.0
        if h.enable_looping_step >= 0:
            return step >= h.enable_looping_step
        return frac >= h.enable_looping_at

    def loop_trigger_label():
        if h.loop_ramp_steps > 0:
            return "ramp"
        return "step" if h.enable_looping_step >= 0 else "frac"

    def should_prewarm_looped_long_seq():
        if h.num_loops <= 0:
            return False
        if h.enable_looping_step >= 0:
            return h.enable_looping_step <= h.iterations
        return h.enable_looping_at <= h.seq_len_bump_frac

    def bump_obs_phase(step):
        if (
            not h.bump_obs_enabled
            or h.bump_obs_every <= 0
            or h.bump_obs_probe_batches <= 0
        ):
            return None
        if h.bump_obs_start_step >= 0 and h.bump_obs_end_step > h.bump_obs_start_step:
            if (
                h.bump_obs_start_step <= step < h.bump_obs_end_step
                and (step - h.bump_obs_start_step) % h.bump_obs_every == 0
            ):
                return "transition"
            return None
        if h.bump_obs_n <= 0:
            return None
        if h.enable_looping_step >= 0:
            bump_step = h.enable_looping_step
        elif max_wallclock_ms is None:
            bump_step = int(math.ceil(h.enable_looping_at * h.iterations))
        else:
            return None
        windows = (
            ("pre", bump_step - h.bump_obs_n, bump_step),
            ("post_early", bump_step, bump_step + h.bump_obs_n),
            (
                "post_late",
                bump_step + h.bump_obs_post_late_offset,
                bump_step + h.bump_obs_post_late_offset + h.bump_obs_n,
            ),
        )
        for phase, start, end in windows:
            if start <= step < end and (step - start) % h.bump_obs_every == 0:
                return phase
        return None

    def loop_policy_probability(step, frac):
        if h.loop_ramp_steps > 0:
            if h.enable_looping_step >= 0:
                center = float(h.enable_looping_step)
                width = float(h.loop_ramp_steps)
                start = center - 0.5 * width
                return min(max((step - start) / max(width, 1.0), 0.0), 1.0)
            if max_wallclock_ms is None:
                center = h.enable_looping_at * h.iterations
                width = float(h.loop_ramp_steps)
                start = center - 0.5 * width
                return min(max((step - start) / max(width, 1.0), 0.0), 1.0)
        return 1.0 if (
            step >= h.enable_looping_step
            if h.enable_looping_step >= 0
            else frac >= h.enable_looping_at
        ) else 0.0

    def sample_looping_active(step, frac):
        p_loop = loop_policy_probability(step, frac)
        if p_loop <= 0.0:
            return False
        if p_loop >= 1.0:
            return True
        return random.random() < p_loop

    def run_bump_probe(step, frac, phase):
        if not h.is_main_process and not h.distributed:
            return
        ...
        def _probe():
            return eval_val(
                ...
                max_batches=h.bump_obs_probe_batches,
            )

        torch.cuda.synchronize()
        t_probe = time.perf_counter()
        cur_loss, cur_bpb = _probe()
        elapsed_ms = 1e3 * (time.perf_counter() - t_probe)
        log(
            f"bump_probe step:{step} frac:{frac:.6f} phase:{phase} "
            f"loop_active:{int(old_looping)} loss:{cur_loss:.8f} "
            f"bpb:{cur_bpb:.8f} eval_time:{elapsed_ms:.0f}ms"
        )
        if h.bump_obs_counterfactual and h.num_loops > 0:
            if old_looping:
                on_loss, on_bpb = cur_loss, cur_bpb
                base_model.looping_active = False
                off_loss, off_bpb = _probe()
            else:
                off_loss, off_bpb = cur_loss, cur_bpb
                if base_model.loop_identity_topology:
                    base_model.clamp_identity_aux_skips_()
                base_model.looping_active = True
                on_loss, on_bpb = _probe()
            base_model.looping_active = old_looping
            p_loop = loop_policy_probability(step, frac)
            policy_loss = (1.0 - p_loop) * off_loss + p_loop * on_loss
            policy_bpb = (1.0 - p_loop) * off_bpb + p_loop * on_bpb
            log(
                f"bump_counterfactual step:{step} frac:{frac:.6f} phase:{phase} "
                f"p_loop:{p_loop:.6f} policy_loss:{policy_loss:.8f} policy_bpb:{policy_bpb:.8f} "
                f"loop_off_loss:{off_loss:.8f} loop_off_bpb:{off_bpb:.8f} "
                f"loop_on_loss:{on_loss:.8f} loop_on_bpb:{on_bpb:.8f} "
                f"delta_loss:{on_loss - off_loss:.8f} delta_bpb:{on_bpb - off_bpb:.8f}"
            )
        else:
            base_model.looping_active = old_looping
        torch.cuda.synchronize()
    ...
    def _grad_norm(t):
        if t.grad is None:
            return 0.0
        return t.grad.detach().float().norm().item()

    def log_bump_layer_stats(step, frac, phase):
        if not h.bump_obs_enabled or not h.bump_obs_layer_stats:
            return
        ...
        for i, block in enumerate(base_model.blocks):
            ...
            grads = (
                None if base_model.qo_bank.grad is None else base_model.qo_bank.grad[i],
                None if base_model.qo_bank.grad is None else base_model.qo_bank.grad[n + i],
                None if base_model.kv_bank.grad is None else base_model.kv_bank.grad[i],
                None if base_model.kv_bank.grad is None else base_model.kv_bank.grad[n + i],
                None if base_model.mlp_up_bank.grad is None else base_model.mlp_up_bank.grad[i],
                None if base_model.mlp_down_bank.grad is None else base_model.mlp_down_bank.grad[i],
            )
            for t in tensors:
                param_sq += _tensor_norm(t) ** 2
            for g in grads:
                if g is not None:
                    grad_sq += g.detach().float().norm().item() ** 2
            for p in block.parameters():
                param_sq += _tensor_norm(p) ** 2
                grad_sq += _grad_norm(p) ** 2
            param_norm = math.sqrt(param_sq)
            grad_norm = math.sqrt(grad_sq)
            log(
                f"bump_layer_stats step:{step} frac:{frac:.6f} phase:{phase} "
                f"layer:{i} looped:{int(h.loop_start <= i <= h.loop_end)} "
    ...
    def step_fn(step, lr_scale, train_forward, obs_phase=None, obs_frac=0.0):
        ...
        muon_momentum = (
            1 - warmup_frac
        ) * h.muon_momentum_warmup_start + warmup_frac * h.muon_momentum
    ...
    if h.warmup_steps > 0:
        ...
        if h.train_seq_len_end != h.train_seq_len:
            ...
            log(
                f"bump_prewarm:seq_len:{old_seq_len}->{train_loader.max_seq_len} "
                f"looping:{int(should_prewarm_looped_long_seq())}"
            )
            _run_cu_bucket_warmup(compiled_train_long, long_train_seq_len)
            if should_prewarm_looped_long_seq():
                base_model.looping_active = True
                _run_cu_bucket_warmup(compiled_train_long, long_train_seq_len)
                for warmup_step in range(h.warmup_steps):
            ...
            for (opt, state) in zip(optimizers, bump_optimizer_states, strict=True):
                opt.load_state_dict(state)
            optimizers.zero_grad_all()
            train_loader.max_seq_len = old_seq_len
            base_model.looping_active = old_looping
            train_loader = DocumentPackingLoader(h, device)
        ...
        _prime_rotary_fixed_slots(
            ...
            (
                (
                    1,
                    max(
                        eval_batch_tokens_local,
                        h.ttt_chunk_size,
                        h.ttt_eval_seq_len,
                        h.global_ttt_batch_seqs * h.eval_seq_len,
                    ),
                    h.ttt_eval_seq_len,
                ),
            ),
