...
def main() -> None:
    ...
    while True:
        ...
        # Log-normal Poisson depth sampling (Geiping et al. 2025): heavy-tailed
    ...
    if master_process:
        torch.save(export_sd, "final_model_noinject.pt")
        model_bytes = os.path.getsize("final_model_noinject.pt")
        code_bytes = len(code.encode("utf-8"))
        log0(f"Serialized model: {model_bytes} bytes")
        log0(f"Code size: {code_bytes} bytes")

        # Save TTT-ready checkpoint with config
        ttt_config = {
        ...
        log0(f"Saved TTT checkpoint: ttt_checkpoint_noinject.pt")
